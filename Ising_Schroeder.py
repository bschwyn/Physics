#Schroeder's Ising Model
#from http://physics.weber.edu/thermal/isingPyTk.html
...
# the coordinates (3,3) are a kludge to elminate a mysterious offset
...
def simulate():
    if running:
        ...
        for step in range(1000):            # (change the number of steps as desired)
            ...
            eDiff = deltaE(i,j)
            if eDiff <= 0 or random.random() < math.exp(-eDiff/T): # Metropolis Algorithm
                s[i,j] = -s[i,j]
                colorSquare(i,j)
